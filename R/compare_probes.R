...
call_specs<-function(kind_counts_file){

  kinds_of_calls<-as.matrix(read.csv(kind_counts_file))
  percent_same<-sum(kinds_of_calls=="S",na.rm = T)/(nrow(kinds_of_calls)*(ncol(kinds_of_calls)-1))
  percent_one<-sum(kinds_of_calls=="O",na.rm = T)/(nrow(kinds_of_calls)*(ncol(kinds_of_calls)-1))
